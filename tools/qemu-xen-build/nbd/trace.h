...
{
    if (trace_event_get_state(TRACE_NBD_REPLY_ERR_UNSUP) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:nbd_reply_err_unsup " "server doesn't understand request %" PRIu32 " (%s), attempting fallback" "\n",
                 qemu_get_thread_id(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 , option, name);
    }
